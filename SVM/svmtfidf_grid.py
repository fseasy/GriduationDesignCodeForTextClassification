    ...
    for c in c_r :
        for bias in bias_range :
            for w_i in range(len(w_positive_range)) :
                ...
                sys.stdout.flush()
                cv_rst.append({'c':c , 'w_positive':w_positive , 'w_negative':w_negative , 'bias':bias , 'positive_prf':positive_prf , 'negative_prf':negative_prf})
...
def certain_handle(cv_rst) :
    ...
    for i in range(limit) :
        rst = sorted_rst[i]
        print 'class : p = %6.2f %% , r = %6.2f %% , f = %6.2f%% . C = %f , bias=%.2f , w_positive = %.2f , w_negative = %.2f' %(
                rst['negative_prf'][0] , rst['negative_prf'][1] , rst['negative_prf'][2] , rst['c'] , rst['bias'] , rst['w_positive'] , rst['w_negative'])
...
def main(pos_f , neg_f , gram_num , cv_num , rst_f ,params) :
    ...
    words_df = calc_allwords_df([pos_words_counter , neg_words_counter])
    ...
    X = build_SVM_format_X_from_docs_words(all_docs,words_df,docs_dict)
...
if __name__ == "__main__" :
    argp = argparse.ArgumentParser(description="svm with tf-idf feature grid search using cross validation")
