...
def build_word_bag(word_bag , training_set_dir ,transdict_for_typename_and_dtype , typename_split_char="." , is_bigram=False) :
    ...
    for file_name in file_list :
        dtype_name = file_name.split(typename_split_char)[0]
        dtype = transdict_for_typename_and_dtype[dtype_name]
        print dtype
        path = os.path.join(training_set_dir , file_name)
        try :
            read_file = open(path)
            for line in read_file :
                #here , every line is a doc
                doc = "doc_" + str(doc_id)
                doc_id += 1
                try :
                    line = line.strip()
                    ...
                    if is_bigram :
                        big_words = []
                        for ite in range(len(valid_words) - 1) :
                            big_words.append("_".join(valid_words[ite:ite+2]))
                        valid_words.extend(big_words)
...
if __name__ == "__main__" :
    if len(sys.argv) <  2 :
        logging.error("usage : %s [%s] [[bigram]]"  %(sys.argv[0] , "target_dir"))
        exit(0)
    wb = word_bag() ;
    load_state = wb.load_word_bag()
    if not load_state :
        is_bigram = False
        if len(sys.argv) == 3 and sys.argv[2] == "bigram" :
            is_bigram = True
        build_word_bag(wb,sys.argv[1],{"postrain":POSITIVE_LABEL,"negtrain":NEGATIVE_LABEL} , is_bigram=is_bigram)
        wb.save_word_bag()
    #wb.dump_to_file() ;
    #wb.dump_feature_DF()
    wb.filter_feature_by_DF(3)
    wb.dump_feature_DF()
    #wb.save_word_bag() 
    wb.calc_all_feature_with_type_CHI(POSITIVE_LABEL)
    wb.get_top_k_feature_with_type_by_CHI(POSITIVE_LABEL , 6000)
    wb.write_feature_with_type(POSITIVE_LABEL)
